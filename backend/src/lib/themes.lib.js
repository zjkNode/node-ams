...
function viewEngine (app, directory) {
  ...
  app.engine('.html', hbs.express4({
    layoutsDir: 'themes/' + directory,
    partialsDir: 'themes/partials/' + directory,
    extname: '.html'
  }));
  app.set('view engine', '.html');
  app.set('views', 'themes/' + directory);

}

exports.init = function (app, callback) {
    (function checkDirectory (directory) {
      fs.stat(path.join(__dirname, '../themes/' + directory), function (err, stats) {
        if (stats && stats.isDirectory()) {
          ...
          if (directory !== 'default') {
            ...
            console.log(directory + '目录不存在或非法');
          }
        }
      });
    })('jdjk-pc');
