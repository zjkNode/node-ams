...
var async = require('async'),
    ...
    productModel = require('../../models/product/product.model');

const { DBError } = require('../../models/errors.model');

exports.add = function(where,callback) {
    mysql.insert(productModel.tbname , where, function(err,resId){
        if(err){
...
exports.delete = function(where, callback){
	mysql.where(where).remove(productModel.tbname,function(err,res){
		if(err){
			logger.errorDB(__filename, err);
			return callback(new DBError());
		}
...
exports.update = function (data, where, callback) {
	mysql.where(where).update(productModel.tbname, data, function(err,res){
		if(err){
			logger.errorDB(__filename, err);
			return callback(new DBError());
		}
...
exports.list = function(where, page, callback){
    async.parallel({
        total:function(callback){
            mysql.where(where).count(productModel.tbname, function(err,res) {
        ...
        list:function(callback){
            mysql.where(where)
                ...
                .select(productModel.tbname, function(err, rows) {
...
exports.one = function(where,callback){
    mysql.where(where).select(productModel.tbname,function(err,rows){
        if(err){
            logger.errorDB(__filename,err);
            return callback(new DBError());
        }
